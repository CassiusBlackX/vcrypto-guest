...
bool vcrypto_fe_protocol_engine_init(char *socket_file_path) {
  ...
  if (rx_ring == NULL) {
    log_error("failed to find the rx_ring: %s with vcrypto_daemon",
              rx_ring_name);
    return false;
  } else {
  ...
  if (tx_ring == NULL) {
    log_error("failed to find the tx_ring: %s with vcrypto_daemon",
              tx_ring_name);
    return false;
  } else {
  ...
  if (sym_crypto_session_pool == NULL) {
    ...
    log_trace("success found sym crypto session pool: %p",
              sym_crypto_session_pool);
  }

  crypto_op_mempool = rte_mempool_lookup(CRYPTO_OP_MP_NAME);
  if (crypto_op_mempool == NULL) {
    log_error("failed to lookup sym crypto op mempool");
    return false;
  } else {
    log_trace("success found sym crypto op mempool: %p", crypto_op_mempool);
  }

  pktmbuf_mempool = rte_mempool_lookup(CRYPTO_MBUF_MP_NAME);
